...
const TestView: React.FC<TestViewProps> = ({ title }) => {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div>
        Here are my dogs:
        {dogNames.map(el=>(<span key={el}>{el}</span>))}
      </div>
          ...
          <div>
            <button onClick={()=>{
              setTodos([...todos, {id: todos.length + 1, content: 'New Todo', completed: false}])
            }}>Add Todo</button>
          </div>
          ...
          <div className="flex">
            ...
            <button
              ...
              onClick={() => todos.length > 0 ? updateTodo(todos[0].id, 'Updated Todo') : null}
            >
              Update Todo
            </button>
          </div>
